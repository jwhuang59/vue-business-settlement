...
export default {
    ...
    methods: {
        getComplianceInfo(){
            this.$request("getComplianceInfo").then((res) => {
                if(res.data.businessLicensePhoto){
                    this.complianceInfo.businessLicensePhoto = res.data.businessLicensePhoto.length > 0 ? res.data.businessLicensePhoto : ['']
                    ...
                    this.complianceInfo.businessApprovalTime = res.data.businessApprovalTime ? res.data.businessApprovalTime : '请选择营业执照核准日期'
        ...
        nextStep(){
            ...
            if(this.complianceInfo.businessLicensePhoto.length < 0){
            ...
            }else if(this.complianceInfo.businessLicenseName === "" || this.complianceInfo.businessLicenseName === undefined || this.complianceInfo.businessLicenseName === null){
                Dialog.alert({message: '请输入营业执照名称'});
                return false;

            }else if(this.complianceInfo.legalRepresentativeName === "" || this.complianceInfo.legalRepresentativeName === undefined || this.complianceInfo.legalRepresentativeName === null){
                Dialog.alert({message: '请输入法定代表人'});
                return false;

            }else if(this.complianceInfo.unifiedCreditCode === "" || this.complianceInfo.unifiedCreditCode === undefined || this.complianceInfo.unifiedCreditCode === null){
                Dialog.alert({message: '请输入统一信用代码'});
                return false;

            }else if(this.complianceInfo.businessRegisteredAddress === "" || this.complianceInfo.businessRegisteredAddress === undefined || this.complianceInfo.businessRegisteredAddress === null){
                Dialog.alert({message: '请输入营业执照注册地址'});
                return false;

            }else if(this.complianceInfo.businessTermValidity === "" || this.complianceInfo.businessTermValidity === undefined || this.complianceInfo.businessTermValidity === null || this.complianceInfo.businessTermValidity === "2"){
                Dialog.alert({message: '请输入营业执照有效期'});
                return false;

            }else if(this.complianceInfo.businessApprovalTime === "" || this.complianceInfo.businessApprovalTime === undefined || this.complianceInfo.businessApprovalTime === null){
                Dialog.alert({message: '请选择营业执照核准日期'});
                return false;

            }else if(this.complianceInfo.businessScope === "" || this.complianceInfo.businessScope === undefined || this.complianceInfo.businessScope === null){
                ...
                Dialog.confirm({
                    ...
                    this.$request("updateComplianceInfo",this.complianceInfo).then((res) => {
                        if(res.data){
                            Toast.success({
                                message:'保存成功',
                                duration:3000,
