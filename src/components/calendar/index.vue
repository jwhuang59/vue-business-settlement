<template>
    <div class="canlendarPanel">
        ...
        <div class="canlendar-main">
            ...
            <ul class="main">
                <li v-for="inx in weekDay" class="disabled">{{ preMonthSize - weekDay + inx }}</li>
                <li v-for="index in monthList[month - 1]">
                    <span :class="{ currentDay: index === day }">{{ index }}</span>
                </li>
...
export default class CanlendarPanel extends Vue {
    ...
    initDate() {
        ...
        // 获得最后一天是星期几，往后填充多少个
        const remineDay = this.getWeekDay(this.year, this.month, this.monthList[this.month - 1]);
